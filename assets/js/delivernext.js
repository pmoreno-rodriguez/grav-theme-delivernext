...
document.addEventListener('DOMContentLoaded', function () {
    const lazySlides = document.querySelectorAll('.modular-row.showcase[data-bg]');
    ...
    if ('IntersectionObserver' in window) {
        const observer = new IntersectionObserver((entries, obs) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    const blend = el.getAttribute('data-blend');
                    const backcolor = el.getAttribute('data-backcolor');

                    if (bg) {
                        el.style.backgroundImage = `url(${bg})`;
                    }
                    if (blend) {
                        el.style.backgroundBlendMode = blend;
                    }
                    if (backcolor) {
                    ...
                    el.removeAttribute('data-blend');
                    el.removeAttribute('data-backcolor');
        ...
        lazySlides.forEach(el => {
            const bg = el.getAttribute('data-bg');
            const blend = el.getAttribute('data-blend');
            const backcolor = el.getAttribute('data-backcolor');

            if (bg) el.style.backgroundImage = `url(${bg})`;
            if (blend) el.style.backgroundBlendMode = blend;
            if (backcolor) el.style.backgroundColor = backcolor;
